    ...
    case FORCESCALC_MESSAGE :
     {
      try
	   ...
	   //изменяем конфигурацию корпуса 7 из 9 степеней свободы
		BodyAnglesCur[0] =  0;//pi/8*sin(t);//продольный сгиб
		BodyAnglesCur[1] =  0;//pi/4*sin(2*t);//первый поперечный сгиб
		BodyAnglesCur[2] =  0;//pi/4*sin(t/3);//второй поперечный сгиб
		BodyAnglesCur[3] =  -pi/4*sin(t/4);//лепесток 1
		...
		for(int i=0;i<6;i++)
		{
			...
			//PrintMatrix(TEMP,"global to seg");
			//PrintScalar(i,"i");
			//PrintVector(temp2,"temp2 segment center in global rf");
			
			
			Mult_m_v_3x1(TRANSPON,TEMP,temp,temp);
			Mult_m_v_3x1(TRANSPON,MainBody,temp,temp);
			//PrintVector(temp,"MainBody^t*TEMP^t*temp");
			
			
			SubVector(temp2,Body[3],temp2);
			if (i==0) 
				{
					PrintVector(temp2,"temp2-Body[3]");
				}
			Mult_m_v_3x1(TRANSPON,MainBody,temp2,temp2);
			//PrintVector(temp2,"MainBody^t*temp2");
			AddVector(temp,temp2,temp);//Радиус вектор ноги в СК корпуса
